...
# Read the dataset
data_comments <- read.csv("Apriorimodel/data/FinalAnnotationSheet.csv")

data_comments <- data_comments %>%
  filter(!(data_comments$`ethical.issue.label.comment.1` == "13. only one issue" & data_comments$`ethical.issue.label.comment.2` != "13. only one issue") &
           !(data_comments$`ethical.issue.label.comment.1` != "13. only one issue" & data_comments$`ethical.issue.label.comment.2` == "13. only one issue"))

#Write the new filtered dataset into a new CSV File
write.csv(data_comments, "Apriorimodel/data/FinalAnnotationSheet_OnlyOneIssueRemoved_Comments.csv", row.names = FALSE)

#Read the new filtered CSV File
data_comments <- read.csv("Apriorimodel/data/FinalAnnotationSheet_OnlyOneIssueRemoved_Comments.csv")

#Print the data to see the new csv file
print(data_comments)

# Remove numbers and leading/trailing spaces from the column values
ethical_issue_label_comment_1 <- gsub("^\\d+\\.\\s*", "", data_comments$`ethical.issue.label.comment.1`)
ethical_issue_label_comment_2 <- gsub("^\\d+\\.\\s*", "", data_comments$`ethical.issue.label.comment.2`)

# Create a list of vectors combining the values of both columns
EthicalIssuesCommentslist <- lapply(1:length(ethical_issue_label_comment_1), function(i) {
  c(ethical_issue_label_comment_1[i], ethical_issue_label_comment_2[i])
})

# Set transaction names
names(EthicalIssuesCommentslist) <- paste("T", c(1:length(ethical_issue_label_comment_1)), sep = "")

# Print names to verify
print(names(EthicalIssuesCommentslist))

# Convert the list to transactions
TransactionEthicalIssuesComments <- as(EthicalIssuesCommentslist, "transactions")

# Print the transactions to verify
print(TransactionEthicalIssuesComments)

# Print the dimensions of the transactions
print(dim(TransactionEthicalIssuesComments))

# Print the item labels of the transactions
print(itemLabels(TransactionEthicalIssuesComments))

# Print the summary of the transactions
print(summary(TransactionEthicalIssuesComments))

# Plot the image of the transactions
image(TransactionEthicalIssuesComments)

# Custom frequency counts considering non-distinct items
all_items <- unlist(EthicalIssuesCommentslist)
...
barplt <- barplot(EthicalIssues_count, main = "",
                  ...
                  ylim = c(0, 1200), names.arg = rep("", length(EthicalIssues_count)), las = 2)
...
relative_freq <- sort(as.numeric(item_freq_sorted/length(TransactionEthicalIssuesComments)),decreasing = TRUE)
...
itemFrequencyPlot(TransactionEthicalIssuesComments,topN=length(item_freq_sorted),type="absolute",col=brewer.pal(8,'Pastel2'), main="Absolute Item Frequency Plot",ylim = c(0, 1000))

itemFrequencyPlot(TransactionEthicalIssuesComments,topN=length(item_freq_sorted),type="absolute",col=brewer.pal(8,'Pastel2'), main="Absolute Item Frequency Plot",ylim = c(0, 500))

itemFrequencyPlot(TransactionEthicalIssuesComments,topN=length(item_freq_sorted),type="relative",col=brewer.pal(8,'Pastel2'),main="Relative Item Frequency Plot",ylim = c(0, 1))

itemFrequencyPlot(TransactionEthicalIssuesComments,topN=length(item_freq_sorted),type="relative",col=brewer.pal(8,'Pastel2'),main="Relative Item Frequency Plot",ylim = c(0, 0.5))


# Apriori model and Association rules
rules <- apriori(TransactionEthicalIssuesComments, 
...
Rule_EthicalIssue_misinformation_RHS <- apriori(TransactionEthicalIssuesComments, 
                                                parameter = list(supp=0.0, conf=0.0, 
                                                                 maxlen=100, 
                                                                 minlen=2),
...
Rule_EthicalIssue_misinformation_LHS <- apriori(TransactionEthicalIssuesComments, parameter=list(supp=0.0, conf=0.0, maxlen=100, 
                                                                      minlen=2),
